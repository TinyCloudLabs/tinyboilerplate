...
export const APP_ID = "xyz.tinycloud.notes";
export const NOTES_SQL_PATH = "notes_index";
export const NOTE_BODY_KV_PATH = "entries/";
export const NOTES_SQL_DATABASE_ID = `${APP_ID}/${NOTES_SQL_PATH}`;
export const NOTE_BODY_KV_PREFIX = `${APP_ID}/${NOTE_BODY_KV_PATH}`;

interface NotesManifest extends Omit<Manifest, "permissions"> {
  manifest_version?: 1;
...
const BACKEND_DELEGATION_NAME = "TinyCloud Notes Backend";
  ...
  return [
    {
      ...
      path: NOTES_SQL_PATH,
      actions: ["read", "write"],
      description: "Read and write TinyCloud Notes metadata in SQL.",
    },
    {
      service: "tinycloud.kv",
      path: NOTE_BODY_KV_PATH,
      actions: ["get", "put", "del", "list"],
      description: "Read and write TinyCloud Notes body content in KV.",
    },
  ];
}

function validateNotesManifest(manifest: Manifest): NotesManifest {
  ...
  if (candidate.defaults !== false) {
    throw new Error("TinyCloud Notes manifest must use defaults: false");
  }
  if (!Array.isArray(candidate.permissions)) {
    throw new Error("TinyCloud Notes manifest must declare explicit permissions");
  }

  return manifest as NotesManifest;
}

export function runtimeManifest(): NotesManifest {
  const raw = JSON.parse(readFileSync(MANIFEST_PATH, "utf-8")) as Manifest;
  return validateNotesManifest(validateManifest(raw));
}
...
function validateBackendPolicy(manifest: NotesManifest): void {
