...
describe("fetchUserInfo", () => {
  ...
  test("throws sanitized error on non-ok response", async () => {
    ...
    await expect(fetchUserInfo("https://example.com", "bad-token")).rejects.toThrow(
      "Failed to fetch user info (404): Not Found",
    );
  });

  test("truncates long error bodies", async () => {
    const longBody = "x".repeat(500);
    globalThis.fetch = (async () => new Response(longBody, { status: 500 })) as typeof fetch;
    try {
  ...
  test("strips control characters from error", async () => {
    ...
    await expect(fetchUserInfo("https://example.com", "token")).rejects.toThrow(
      "Failed to fetch user info (400): errormessage",
    );
  });

  test("returns UserInfo on success", async () => {
    globalThis.fetch = (async () =>
      new Response(JSON.stringify({ sub: "user123", address: "0xABC" }), {
        status: 200,
