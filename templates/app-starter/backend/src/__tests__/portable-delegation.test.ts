...
describe("activatePortableDelegation", () => {
  it("activates each resource in a multi-resource portable delegation", async () => {
        ...
        resources: [
          {
            ...
            space: "applications",
            path: "xyz.tinycloud.app-starter/probe/",
            actions: ["tinycloud.kv/get", "tinycloud.kv/put"],
          },
          {
            service: "tinycloud.sql",
            space: "applications",
            path: "xyz.tinycloud.app-starter/auxiliary_index",
    ...
    expect(calls.map((call: any) => call.path)).toEqual([
      "xyz.tinycloud.app-starter/probe/",
      "xyz.tinycloud.app-starter/auxiliary_index",
    ]);
  ...
  it("routes same-service KV resources by path instead of leaving the last handle active", async () => {
      ...
      node as any,
      {
        ...
        resources: [
          ...
          },
          {
            ...
            path: "secrets/xyz.tinycloud.app-starter/",
    ...
    await (access as any).kv.get("xyz.tinycloud.app-starter/probe/value");
    await (access as any).kv.get("secrets/xyz.tinycloud.app-starter/api-key");

    expect(calls).toHaveLength(2);
    expect(kvGets.map(({ label }) => label)).toEqual(["app-data", "secrets-space"]);
    expect(kvGets.map(({ key }) => key)).toEqual([
      "xyz.tinycloud.app-starter/probe/value",
      "secrets/xyz.tinycloud.app-starter/api-key",
    ]);
          ...
          resources: [
            {
              service: "tinycloud.sql",
              space: "applications",
              path: "xyz.tinycloud.app-starter/primary_index",
              actions: ["tinycloud.sql/read"],
            },
            {
              service: "tinycloud.sql",
              space: "applications",
              path: "xyz.tinycloud.app-starter/audit_index",
              actions: ["tinycloud.sql/read"],
            },
          ],
        } as any,
