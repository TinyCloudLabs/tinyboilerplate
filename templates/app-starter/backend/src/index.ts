...
import { createAppStarterBackendIdentity } from "./startup.js";
...
const FRONTEND_URL =
  ...
  (HTTPS_CERT_FILE && HTTPS_KEY_FILE ? "https://localhost:5175" : "http://localhost:5175");
...
async function main() {
  const { node, did } = await createAppStarterBackendIdentity({
  ...
  app.use("/api/probe", authMiddleware, delegationMiddleware, createProbeRouter());
  ...
  const server = tlsConfig
    ? createHttpsServer(tlsConfig, app).listen(PORT, () => {
        console.log(`TinyCloud App Starter backend ready: https://localhost:${PORT}`);
      })
    : app.listen(PORT, () => {
        console.log(`TinyCloud App Starter backend ready: http://localhost:${PORT}`);
      });
...
main().catch((error) => {
  console.error("Failed to start TinyCloud App Starter backend:", error);
  process.exit(1);
});
