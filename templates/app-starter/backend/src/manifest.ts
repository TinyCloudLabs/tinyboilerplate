...
export const PROBE_KV_PATH = "probe/";
export const PROBE_KV_PREFIX = `${APP_ID}/${PROBE_KV_PATH}`;

interface AppStarterManifest extends Omit<Manifest, "permissions"> {
  manifest_version?: 1;
...
function validateAppStarterManifest(manifest: Manifest): AppStarterManifest {
  ...
  return manifest as AppStarterManifest;
}

export function runtimeManifest(): AppStarterManifest {
  const raw = JSON.parse(readFileSync(MANIFEST_PATH, "utf-8")) as Manifest;
  return validateAppStarterManifest(validateManifest(raw));
}

function resolvePermissions(
  manifest: AppStarterManifest,
...
function validateBackendPolicy(manifest: AppStarterManifest): void {
